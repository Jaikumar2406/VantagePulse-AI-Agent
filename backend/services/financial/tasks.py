import asyncio

from agents.Financial_Estimation_Agent import run_financial_estimation_agent
from backend.core.database import db


def process_financial_background(startup_id: str):
    try:
        print(f"[{startup_id}] Running Financial Estimation Agent...")
        system_state = db[startup_id]["result"]
        updated_system_state = asyncio.run(run_financial_estimation_agent(system_state))
        db[startup_id]["result"] = updated_system_state
        print(f"[{startup_id}] Financial estimation completed")
    except Exception as e:
        db[startup_id]["status"] = "error"
        db[startup_id]["error"] = str(e)
        print(f"[{startup_id}] Error:", str(e))
